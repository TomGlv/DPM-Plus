...
def calculate_master_distance(tier, rank, lp):
    # Mapping des paliers vers le nombre de LP totaux nécessaires pour le Master
    tiers = {"DIAMOND": 0, "EMERALD": 400, "PLATINUM": 800} # On part du Master
    ranks = {"IV": 300, "III": 200, "II": 100, "I": 0}
    
    if tier != "DIAMOND": # Simplification pour ton cas actuel
        return "Calcul disponible en Diamond"
        
    lp_to_master = ranks.get(rank, 0) + (100 - lp)
    wins_needed = -(-lp_to_master // 20) # Arrondi supérieur
...
def main():
    ...
    if os.path.exists(FILE_NAME):
        with open(FILE_NAME, "r") as f:
            if f.read().strip() == last_match_id: return
    ...
    if game and rank_info:
        ...
        embed = {
            "embeds": [{
                ...
                "footer": {"text": f"Mamad0uBalTr0u • {last_match_id}"},
                "timestamp": time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime())
            }]
        }
        
        if requests.post(WEBHOOK, json=embed).status_code in [200, 204]:
            with open(FILE_NAME, "w") as f: f.write(last_match_id)
